...
def with_payload(df: DataFrame) -> DataFrame:
    """Add a `payload` column by parsing the `json_payload` string column as JSON."""
    spark = df.sparkSession
    # Sample one row to infer the schema cheaply (avoids a full dataset scan)
    sample_df = (
    ...
    # Infer schema from the sample, then parse the full json_payload column
...
def parse_bronze_topic(raw_events: DataFrame, topic: str) -> DataFrame:
    """Filter `raw_events` to a single topic; payload is left unparsed to save memory."""
    return raw_events.filter(F.col("event_source") == topic)


def parse_cdc_table(spark: SparkSession, table: str) -> DataFrame:
    """Read a Bronze CDC Iceberg table directly by its fully-qualified name."""
    return spark.table(table)


def unix_ms_to_ts(col: F.Column) -> F.Column:
    """Convert a Unix epoch milliseconds column to a Spark TIMESTAMP."""
...
def scd2_from_events(
    ...
    """Produce SCD Type-2 rows by detecting state changes over time."""
    # Window partitioned by the business key, ordered by the event sequence
    order_window = Window.partitionBy(*key_cols).orderBy(*ordering_cols)
    # Hash the state columns to detect changes cheaply (no column-by-column comparison)
    state_hash = F.sha2(
        F.concat_ws("||", *[F.coalesce(F.col(col).cast("string"), F.lit("")) for col in state_cols]),
        256
    )
    # Keep only the first record, or any record where the state hash changed
    deltas = (events
    ...
    # valid_to = start of the next version, or a far-future sentinel if this is the current version
    return (deltas
        .withColumn("valid_to",
            F.coalesce(
...
def surrogate_key(*cols: Column) -> Column:
    """Generate a stable surrogate key using Spark's xxhash64 function."""
