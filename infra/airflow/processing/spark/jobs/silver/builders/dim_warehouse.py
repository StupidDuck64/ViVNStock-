"""Builder for the warehouse dimension (`dim_warehouse`): metadata + inventory stats."""
...
def build_dim_warehouse(spark: SparkSession, _: DataFrame | None) -> DataFrame:
    # Warehouse metadata (name/region/country) from the warehouses CDC table
    ...
    # Inventory movement rows: used to compute total qty/reserved per warehouse per timestamp
    ...
    # Aggregate inventory metrics per warehouse per change_ts
    ...
    # Keep only the most recent metadata row for each warehouse_id
    ...
    # Apply SCD2 to track warehouse state changes over time
