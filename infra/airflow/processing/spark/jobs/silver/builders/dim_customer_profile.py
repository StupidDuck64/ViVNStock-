...
def build_dim_customer_profile(spark: SparkSession, _: DataFrame | None) -> DataFrame:
    # Read two CDC sources and combine them to form a complete customer profile
    ...
    # Normalise the schema so users and segments can be unioned on the same columns
    ...
    # Collapse simultaneous changes and retain the latest non-null value per key
    ...
    # Forward-fill attributes to carry the last known state across the timeline
