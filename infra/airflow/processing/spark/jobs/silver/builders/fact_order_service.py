...
def build_fact_order_service(spark: SparkSession, raw_events: DataFrame | None) -> DataFrame:
    # This fact requires raw_events from Bronze
    ...
    def extract_latest_by_topic(topic: str, extract_fields: dict) -> DataFrame:
        """Extract and return the latest record per order_id for a given topic."""
    ...
    # Join orders + payments + shipments into one fine-grained fact row per order_id
    fact = (orders.alias("o")
    ...
    # Temporal join: resolve surrogate keys using valid_from / valid_to range
    enriched = (fact
        .join(
            customers,
