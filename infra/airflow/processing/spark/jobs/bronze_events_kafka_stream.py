...
def build_stream(
    ...
    *,
    ...
    # Normalise topic list from CSV string
    ...
    # Decode Avro bytes into a JSON string so downstream jobs can parse the payload
...
def write_events(df: DataFrame, *, table: str, checkpoint: str) -> None:
    # Append the stream to an Iceberg table; stop automatically when the backlog is consumed
    query = (
...
class _ProgressListener(StreamingQueryListener):
    # Log batch progress events to track throughput and query state
...
def main() -> None:
    ...
    # Build a Spark session from the shared utility
