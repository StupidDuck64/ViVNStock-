...
def build_stream(
    ...
    *,
    ...
    # Read from Kafka with a per-micro-batch offset cap to control resource usage
    src = (
    ...
    # UDF: decode Avro bytes → JSON string using the Schema Registry
...
def write_stream(df: DataFrame, *, table: str, checkpoint: str) -> None:
    # availableNow: process all existing data then auto-stop (bounded streaming)
...
def main() -> None:
    ...
    # Embed the target table in the app name for easy identification in the Spark UI
