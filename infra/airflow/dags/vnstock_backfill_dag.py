"""
VNStock Backfill DAG

Orchestrates two Spark backfill jobs:
  1. backfill_1d  — Daily OHLCV from 2015-01-01 to today
                    Source: DNSE REST API (resolution=1D)
                    Target: iceberg.bronze.vnstock_ohlc_1d
                    Schedule: Daily 02:00 ICT (incremental, high-water mark)

  2. backfill_1m  — 1-minute OHLCV + derived aggregations
                    Source: VCI (primary) + DNSE (fallback)
                    Targets: bronze.vnstock_ohlc_1m, 5m, 15m, 30m, 1h, 4h
                    Schedule: Daily 01:00 ICT (incremental, high-water mark)

Both jobs pass ALL symbols (HOSE + HNX + UPCOM + Derivative + Index)
via the VNSTOCK_SYMBOLS environment variable gathered from all per-board vars.

Run modes:
  --run-mode init   → createOrReplace (full rebuild, used ad-hoc)
  --run-mode daily  → append with high-water mark (scheduled)

DAG design:
  ┌────────────────────────────────────┐
  │ check_api_key  (PythonOperator)    │  Fail-fast: assert DNSE_API_KEY is set
  └──────────┬─────────────────────────┘
             │
  ┌──────────▼──────────────────────────────────────────┐
  ...
  └──────────┬──────────────────────────────────────────┘
             │
  ┌──────────▼──────────────────────────┐
  │ maintenance_1d  (PythonOperator)    │  expire snapshots > 30d
  └──────────┬──────────────────────────┘
             │
  ┌──────────▼──────────────────────────────────────────┐
  │ backfill_1m  (SparkSubmitOperator)                  │
  │   --run-mode daily  --days 3                        │
  └──────────┬──────────────────────────────────────────┘
             │
  ┌──────────▼──────────────────────────┐
  │ maintenance_1m  (PythonOperator)    │  expire snapshots > 30d
  └─────────────────────────────────────┘

Environment variables consumed from .env / docker-compose:
  DNSE_API_KEY                — required, passed to Spark job via env_vars
  VNSTOCK_SYMBOLS_HOSE        — 403 symbols
  VNSTOCK_SYMBOLS_HNX         — 296 symbols
  VNSTOCK_SYMBOLS_UPCOM       — 300 symbols
  VNSTOCK_SYMBOLS_DERIVATIVE  — 4 symbols
  VNSTOCK_SYMBOLS_INDEX       — 9 symbols
"""
